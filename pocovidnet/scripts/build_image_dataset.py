...
if __name__ == "__main__":
    ...
    parser.add_argument('-out', type=str, default="../data/lus-optimal-pocovidnet")
    ...
    for mode in TAKE_MODE:
        for fp in os.listdir(os.path.join(POCUS_IMAGE_DIR, mode)):
            if fp[-3:] in ["png", "jpg", "peg", "JPG", "PNG"]:
                # LUS: updated labels
                label_dir = label_to_dir_lus(fp[:3])
                if label_dir in TAKE_CLASSES_LUS:
                    shutil.copy(
    ...
    for mode in TAKE_MODE:
        ...
        for i in range(len(vid_files)):
            ...
            # determine label
            # LUS: updated labels
            label = label_to_dir_lus(vid_files[i][:3])
            if label not in TAKE_CLASSES_LUS:
                continue
